...
afs_ProcessOpRename(struct vcache *avc, struct vrequest *areq)
{
    ...
    do {
	tc = afs_Conn(&old_pdir_fid, areq, SHARED_LOCK, &rxconn, RWONLY);
    	if (tc) {
		    ...
		    afs_ucred_t *acred)
{
    ...
    do {
	tc = afs_Conn(&tdp->f.fid, areq, SHARED_LOCK, &rxconn, RWONLY);
	if (tc) {
	    switch (vType(avc)) {
	    case VREG:
                /* Make file on server. */
...
afs_ProcessOpRemove(struct vcache *avc, struct vrequest *areq)
{
	...
	do {
	    tc = afs_Conn(&pdir_fid, areq, SHARED_LOCK, &rxconn, RWONLY);
	    if (tc) {
...
afs_ResyncDisconFiles(struct vrequest *areq, afs_ucred_t *acred)
{
	...
	if (tvc->f.ddirty_flags & VDisconRemove) {
  	...
  	do {
	    tc = afs_Conn(&tvc->f.fid, areq, SHARED_LOCK, &rxconn, RWONLY);
  	    if (tc) {
